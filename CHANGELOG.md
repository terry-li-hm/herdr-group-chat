...
- **In-room layouts close the emptied backstage workspace**: after a successful `grid`, `grid2` or `quad` placement the launcher closes `agents · group-chat` once no pane in it reports an agent; `compact` keeps it.
