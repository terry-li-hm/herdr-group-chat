...
def test_hook_notice_rendering_after_later_participant_startup_is_dismissed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A notice that renders only after the startup window — while later
    participants start — is still dismissed with esc, never with trust."""
    monkeypatch.setattr(
        module,
        "PARTICIPANTS",
        (("pi", "pi-peer"), ("codex", "codex-peer"), ("grok", "grok-peer")),
    )
    monkeypatch.setattr(module, "HOOK_DISMISS_INTERVAL_S", 0)
    calls: list[list[str]] = []
    codex_reads: list[list[str]] = []
    ...
    def fake_run_json(_herdr_bin: str, arguments: list[str], timeout: float | None = 30) -> dict:
        ...
        if arguments[:2] == ["pane", "process-info"]:
            return {"result": {"process_info": {"foreground_processes": [{"name": "zsh"}]}}}
        if arguments[:2] == ["pane", "read"] and arguments[2] == "w-agents:p-codex":
            codex_reads.append(arguments)
            if len(codex_reads) <= module.HOOK_DISMISS_ATTEMPTS:
                return {"result": {"read": {"text": ""}}}
            return {
                "result": {
                    "read": {
                        "text": "Codex detected unreviewed lifecycle hooks.\n"
                        "Press t to trust all; enter to review hooks; esc to close"
                    }
                }
            }
    ...
    assert failures == []
    # The startup window exhausted itself before grok started; the notice was
    # only visible to the post-startup pass, which still dismissed it with esc.
    grok_start = next(
        index for index, call in enumerate(calls) if call[:3] == ["agent", "start", "grok-peer"]
    )
    assert len(codex_reads) == module.HOOK_DISMISS_ATTEMPTS + 1
    last_codex_read = max(
        index
        for index, call in enumerate(calls)
        if call[:3] == ["pane", "read", "w-agents:p-codex"]
    )
    assert last_codex_read > grok_start
    key_sends = [call for call in calls if call[:2] == ["agent", "send-keys"]]
    assert key_sends == [["agent", "send-keys", "codex-peer", "esc"]]
    assert calls.index(key_sends[0]) > grok_start
    assert not any("t" in call for call in key_sends)
